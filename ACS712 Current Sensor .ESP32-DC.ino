...
float zeroCurrent = 1.360; // Adjust this based on no-load DC readings

void setup() {
  Serial.begin(115200);
  Serial.println("ACS712 DC Current Sensor Test");
...
void loop() {
  ...
  Serial.print("Measured DC Current: ");
...
float readCurrent() {
  ...
  for (int i = 0; i < samples; i++) {
    average += ACS.mA_DC();  // Read DC current in milliamps
